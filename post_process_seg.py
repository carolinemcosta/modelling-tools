...
def smooth_segmentation(segsmooth_bin, original_seg, smooth_seg, tags_list, target_res, fuzz_factor=1.0, voxel_padding=10):
    ...
    """ Smooth segmentation using segsmooth. Segsmooth is part of the segtools package (MUG). Expects NRRD files.
        ...
        Parameters:
          ...
          target_res (float): target resolution in millimeters. Ex. 0.25
    ...
    if os.path.isfile(original_seg) and not os.path.isfile(smooth_seg):
        ...
        cmd = "{} {} {} -l {} --fuzz-factor {} --voxel-units --padding {} -r {}".format(
            segsmooth_bin,
            original_seg,
            smooth_seg,
...
def tag_base_with_plane(segclip_bin, original_seg, base_seg, plane_name, tags_list, base_tag):
    ...
    if os.path.isfile(original_seg) and not os.path.isfile(base_seg):
        for tag in tags_list:
            print("Tagging base of tag {:d}...".format(tag))

            cmd = "{} {} {} --clip-tag {} --to-tag {} --origin {} --normal {}".format(segclip_bin,
                                                                                      original_seg,
                                                                                      base_seg,
                                                                                      tag,
                                                                                      base_tag,
...
def add_bath_to_seg(segchangetag_bin, original_seg, bath_seg, bath_tag):
    ...
    if os.path.isfile(original_seg) and not os.path.isfile(bath_seg):
        ...
        cmd = "{} {} {} 0 {:d}".format(segchangetag_bin, original_seg, bath_seg, bath_tag)
        os.system(cmd)
