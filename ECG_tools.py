...
def get_torso_ecg(phie_file, elec_name):
    """ Create a 12-lead ECG from torso electrode data including time step data
      Parameters:
        ...
        elec_name (string): Name of electrodes file
      Return:
        ecg (dict): 12-lead ECG
        t_steps (list): time steps
...
def write_ecg_to_data_file(file_name, ecg, t_steps):
    """ Write ECG leads as numerical data only using Numpy
        Format: t_steps LI LII LIII aVR aVL aVF V1 V2 V3 V4 V5 V6
      Parameters:
...
def get_electrodes_from_phie_rec_file(phie_file):
    ...
    igb_obj = igb.IGBFile(phie_file)
    header = igb_obj.header()
    data = igb_obj.data()
    igb_obj.close()

    n_traces = header.get('x')
    n_time_steps = header.get('t')
    ...
    data_reshaped = np.reshape(data[0:n_time_steps*n_traces], (n_time_steps, n_traces))

    t_steps = np.linspace(0, dim_t, n_time_steps)
...
def plot_ecg_qrs_qt(t_steps, ecg, qrs_start, qrs_end, qt_int, show_fig, fig_name, y_lim=Tuple[float, float]):
    """ Plot all ECG leads with QRSd and QT as horizontal lines
      Parameters:
        ...
        y_lim (tuple): 2 elements tuple with y-axis lower and upper limits on plot
