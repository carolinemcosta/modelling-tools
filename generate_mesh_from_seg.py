...
def create_CGAL_config_file(script_name, image_name, mesh_name, myo_res, bath_res, bath_tag, 
  ...
  if rv_pool_tag < 0: rv_pool_tag = bath_tag
  if lv_pool_tag < 0: lv_pool_tag = bath_tag
  
  # build dictionary with config parameters
  config = {"input_img": image_name,
            "output_carp": %mesh_name,
            "output_vtk": "%s.vtk"%mesh_name,
...
def convert_nrrd_to_inr(segconvert_bin, image_name):
  ...
  if os.path.isfile(image_name) and not os.path.isfile(inr_image):
    ...
    cmd = "%s %s %s" % (segconvert_bin, image_name, inr_image)
    os.system(cmd)  
    

def generate_cgal_multilabel_mesh(cgal_bin, mesh_name, config_script):
  
  ''' Generates CGAL mesh from a multilabel segmentation. 
  ...
  if not os.path.isfile(mesh_name + ".elem"):
    ...
    cmd = "%s %s"%(mesh_name, config_script)
    os.system(cmd)  
...
def smooth_myo_surface(meshtool_bin, mesh_name, smooth_mesh, myo_tags):
  ...
  if os.path.isfile(mesh_name + ".elem") and not os.path.isfile(smooth_mesh + ".elem"):
    ...
    cmd = "%s extract surface -msh=%s -surf=%s-outer -op=%s -ofmt=carp_txt"%
          (meshtool_bin, mesh_name, mesh_name, 
           " ".join(str(t) for t in myo_tags))
          
    os.system(cmd)
    
    # smooth the surface surrounding the myocardium
    cmd = "%s smooth surface -msh=%s -surf=%s-outer -outmsh=%s -ofmt=carp_txt -thr=0.1 -smth=0.15 -iter=50"%
          (meshtool_bin, mesh_name, mesh_name, smooth_mesh)
